...
sqrt(9)
sqrt(x=9) #x는 함수의 매개변수로 인식 따라서 글로벌변수로 처리된것이 아니다.
x
sqrt(x <- 9) #x라는 변수로 활용
x
...
mytext
mytext[1]
mytext[1,2] #matix에 사용하므로 벡터에서 쓸 수 없다.
...
1:3
1:100
mytext[-1] #1번요소 제외한 나머지지
mytext[c(-1:-3)]
mytext[-c(1:3)]

#TRUE와 FALSE를 이용해서 작업하기
mytext[c(T,T,F,F)] #TRUE로 표현한 요소만
mytext[mytext=="java"] #조건을 만족하는 요소만 출력
numlist <- 1:100
numlist
numlist %%2==0 # %% : 나머지 구하기
...
v1 <- c(70,80,90,100)
v1
names(v1) # name는 벡터의 각요소의 이름을 출력
